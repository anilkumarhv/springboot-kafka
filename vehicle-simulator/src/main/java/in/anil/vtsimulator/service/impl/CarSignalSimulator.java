...
import in.anil.vtsimulator.service.SimulationService;
...
public class CarSignalSimulator implements SimulationService {
    ...
    public void simulate(int vehicleId, int emitInterval, int stopIndex) {
        ...
        for(short i =0 ; i < availbleLocations.size() ; i++){
            ctr = getCtr(vehicleId, emitInterval, stopIndex, ctr, i);
    ...
    private int getCtr(int vehicleId, int emitInterval, int stopIndex, int ctr, short i) {
        ctr++;
        if (ctr == stopIndex) {
    ...
    private void emitSignal(long vehicleId, VehicleLocation location) {
        log.info("Vehicle Id : {}  GPS : {}", vehicleId, location);
        location.setVehicleId(vehicleId);
        producer.produce(vehicleId,location);
    }

    private void emmitOfflineSignal(long vehicleId, VehicleLocation location) {
        log.info("Vehicle Id : {}  GPS : {}", vehicleId, location);
        location.setVehicleId(vehicleId);
    ...
    private void initialiseLocations() {
        availbleLocations = new ArrayList<>();
